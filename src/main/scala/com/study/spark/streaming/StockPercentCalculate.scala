...
object StockPercentCalculate {
	...
	def main(args: Array[String]): Unit = {
		...
		event.foreachRDD(jsonArray => {
			jsonArray.foreachPartition(partitions => {
				...
				if (!partitions.isEmpty) {
					...
					partitions.foreach(msg => {
						/*val sqlData = "insert into push_data_receive_log(content,sys_create_date) "+ "values('"  + msg +"'" + "," +  "'"+   msg.get("timestamp") +"'" + ")"
						val stmtPush = connPush.createStatement()
						stmtPush.executeUpdate(sqlData)*/

						val stockPercent = msg.get("stockPercent").toString().toDouble
						val userPercent = msg.get("userPercentSetting").toString().toDouble
						val state = msg.get("state").toString.toByte
						val stockCode = msg.get("stockCode").toString
						val userId: String = msg.get("userId").toString
						val stockName = broadcastVal.value.get(msg.get("stockCode").toString).get
						val stockCodeUsual = stockCode.substring(0, stockCode.lastIndexOf("."))

						if (state == 0 && (stockPercent <= userPercent)) {
						...
						//	redisStockPushClient.del(PushRedisConstants.STOCK_PUSH_ELF_PERCENTAGE_DOWN_USER_SET + stockCode)
							...
							val sqlPush = "insert into push_log(stock_code,user_id,drop_percent,percent_now,sys_create_time) " + "values('" + stockCode + "'" + "," + userId + "," + userPercent + "," + stockPercent + "," + "'" +  msg.get("timestamp") + "'" + ")"
							val stmtPush = connPush.createStatement()
							stmtPush.executeUpdate(sqlPush)

						}
						else if (state == 1 && stockPercent >= userPercent) {
						...
						//	redisStockPushClient.del(PushRedisConstants.STOCK_PUSH_ELF_PERCENTAGE_UP_USER_SET + stockCode)
							...
							val sqlPush = "insert into push_log(stock_code,user_id,inc_percent,percent_now,sys_create_time) " + "values('" + stockCode + "'" + "," + userId + "," + userPercent + "," + stockPercent + "," + "'" + msg.get("timestamp") + "'" + ")"
							val stmtPush = connPush.createStatement()
							stmtPush.executeUpdate(sqlPush)
						}
					})
					//		redisStockPushClient.close()
					connPush.close()
					redisStockPushClient.close()
	...
	private def getPercentUpContent(stockName: String, stockCode: String, stockPercent: Double, stockPercentSetting: Double)

	= "小沃盯盘提醒您：您的自选股 " + stockName + "(" + stockCode + ")" + DateUtils.getCurrentFormatTime("yyyy-MM-dd HH:mm:ss") + "涨到" + StringUtils.formatDouble(stockPercent) + " %, 超过您设置的 " + stockPercentSetting + "%，更多信息请点击查看详情"

	private def getPercentDownContent(stockName: String, stockCode: String, stockPercent: Double, stockPercentSetting: Double)

	= "小沃盯盘提醒您：您的自选股 " + stockName + "(" + stockCode + ")" + DateUtils.getCurrentFormatTime("yyyy-MM-dd HH:mm:ss") + "跌到" + StringUtils.formatDouble(stockPercent) + " %, 超过您设置的 " + stockPercentSetting + "%，更多信息请点击查看详情"
