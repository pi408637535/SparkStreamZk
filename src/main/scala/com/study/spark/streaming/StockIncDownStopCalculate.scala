...
object StockIncDownStopCalculate {
	...
	//	val paras = Array("192.168.152.137:9092,192.168.152.160:9092,192.168.152.163:9092", "incDownStop")
	    val paras = Array("192.168.1.226:9092,192.168.1.161:9092,192.168.1.227:9092", "incDownStopOld")
			...
			jsonArray.foreachPartition(iterator=>{
				...
				iterator.foreach(iteratorElement=>{
					...
					for(i <-0 to iteratorElement.size() - 1 ){
						...
						if(stockPriceClose == stockPriceHigh ){
							val jsonResult = new JSONObject();
							jsonResult.put("stockCode", stockCode)
							jsonResult.put("stockPriceClose", stockPriceClose)
							jsonResult.put("stockCode", stockCode)
					...
					val message=new ProducerRecord[String, String]("incDownStopResult",null,jsonArray.toString)
