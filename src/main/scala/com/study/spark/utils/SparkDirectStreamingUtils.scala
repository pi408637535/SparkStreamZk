...
/**
  ...
  */
object SparkDirectStreamingUtils {


  val log = org.apache.log4j.LogManager.getLogger("SparkDirectStreamingUtils")
  ...
  def createStreamingContext(appName:String,bootStrapServer:String,zkServerIp:String,zkAddress:String,topics:String,processTime:Long):(StreamingContext,InputDStream[(String,String)])={
    ...
    var kafkaParams=Map[String,String]("bootstrap.servers"-> bootStrapServer)//创建一个kafkaParams
    ...
    val zkOffsetPath= zkAddress //zk的路径
    val topicsSet= topics.split(",").toSet//topic名字

    val ssc=new StreamingContext(sparkConf,Seconds(processTime))//创建StreamingContext,每隔多少秒一个批次
